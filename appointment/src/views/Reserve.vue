  ...
  <div v-if="toreserve" class="results">
    <div
      ...
      <div>
        <h6>{{ key }}</h6>
        <div v-if="item == 'false'">
          <button @click="reserve_it(this.date+' '+key.slice(0, 2)+':00:00')">reserve now</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async reserve_it(date){
      console.log(date)
      let token = localStorage.getItem("token");
      const res = await fetch(
        "https://localhost/appointment-manager/app/appointment/reserve",
        {
          method: "POST",
          headers: {
